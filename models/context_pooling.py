...
class ContextPoolingGraph:
    ...
    def __init__(self, triples, n_ent, n_rel, device):
        ...
        self.src = all_edges[:, 0]
        self.rel = all_edges[:, 1]
        self.dst = all_edges[:, 2]
    ...
    def _csr_get_flat_indices(self, starts, lengths):
        """
        Vectorized generation of range indices for CSR expansion.
        Replaces slow loop: [arange(s, s+l) for s, l in zip(starts, lengths)]
        """
        total_edges = lengths.sum().item()
        if total_edges == 0:
            return torch.empty(0, device=self.device, dtype=torch.long)
            
        base_indices = starts.repeat_interleave(lengths)
        
        ones = torch.ones(total_edges, device=self.device, dtype=torch.long)
        cumsum_len = torch.cumsum(lengths, 0)
        
        if len(lengths) > 1:

            boundaries = cumsum_len[:-1]
            ones[boundaries] -= lengths[:-1]

        inner_offsets = torch.cumsum(ones, 0) - 1
        
        return base_indices + inner_offsets
    ...
    def get_neighbors_full(self, nodes, q_sub=None, q_rel=None):
        ...
        active_batch_ids = batch_ids[mask_non_zero]
        active_node_ids = node_ids[mask_non_zero]
        
        batch_ids_rep = active_batch_ids.repeat_interleave(active_lengths)
        src_ids_rep = active_node_ids.repeat_interleave(active_lengths)
    ...
    def get_cp_neighbors(self, nodes, query_relations, cp_tensor, head_idx_map, tail_idx_map, complement=False, q_sub=None, q_rel=None):
        ...
        src_ids_rep = src_ids_rep[valid_indices]
        ...
        mask_loop = sampled_edges[:, 2] == (self.n_rel * 2)
        _, old_idx = head_index[mask_loop].sort()
        old_nodes_new_idx = tail_index[mask_loop][old_idx]
        
        sampled_edges = torch.cat([sampled_edges, head_index.unsqueeze(1), tail_index.unsqueeze(1)], 1)
        tail_nodes, _ = torch.unique(sampled_edges[:, [0, 3]], dim=0, sorted=True, return_inverse=True)
...
class ContextPooling(KGModel):
    ...
    def __init__(self, nentity, nrelation, base_dim, gamma, 
                 n_layer=2, attn_dim=5, dropout=0.1, act='relu', 
                 train_triples=None, data_path=None, **kwargs):
        ...
        self.act = {'relu': nn.ReLU(), 'tanh': torch.tanh, 'idd': lambda x: x}[act]
        ...
        self.gnn_layers = nn.ModuleList([GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, nrelation, self.act) for _ in range(self.n_layer)])
        self.acc_layers = nn.ModuleList([GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, nrelation, self.act) for _ in range(self.n_layer)])
        self.rec_layers = nn.ModuleList([GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, nrelation, self.act) for _ in range(self.n_layer)])
        self.acc_c_layers = nn.ModuleList([GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, nrelation, self.act) for _ in range(self.n_layer)])
        self.rec_c_layers = nn.ModuleList([GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, nrelation, self.act) for _ in range(self.n_layer)])
    ...
    def _sliced_gru_step(self, x, h_prev, crop_dim):
        ...
        def get_sliced_w(w_full, in_dim_slice):
            w_r = w_full[idx_r, :in_dim_slice]
            w_z = w_full[idx_z, :in_dim_slice]
            w_n = w_full[idx_n, :in_dim_slice]
            return torch.cat([w_r, w_z, w_n], dim=0)

        def get_sliced_b(b_full):
            b_r = b_full[idx_r]
            b_z = b_full[idx_z]
            b_n = b_full[idx_n]
            return torch.cat([b_r, b_z, b_n], dim=0)
    ...
    def forward_gnn(self, subs, rels, crop_dim=None, mode='train'):
        ...
        for i in range(self.n_layer):
            ...
            nodes_full, edges_full, old_nodes_new_idx = self.graph.get_neighbors_full(
                nodes, q_sub=mask_q_sub, q_rel=mask_q_rel
            )
            ...
            def run_stream(layer, q_rels_prev, tensor, complement):
                _, edges, _ = self.graph.get_cp_neighbors(
                    nodes, q_rels_prev, tensor, head_idx_map, tail_idx_map, 
                    complement=complement, q_sub=mask_q_sub, q_rel=mask_q_rel
                )
                if edges.size(0) == 0:
                     return torch.zeros_like(h_full)
                return layer(q_sub, q_rel, hidden, edges, nodes_full.size(0), crop_dim=crop_dim)

            hidden_list.append(run_stream(self.acc_layers[i], q_rels_acc, self.accuracy_tensor, False))
            hidden_list.append(run_stream(self.rec_layers[i], q_rels_rec, self.recall_tensor, False))
            hidden_list.append(run_stream(self.acc_c_layers[i], q_rels_acc_c, self.accuracy_tensor, True))
            hidden_list.append(run_stream(self.rec_c_layers[i], q_rels_rec_c, self.recall_tensor, True))
