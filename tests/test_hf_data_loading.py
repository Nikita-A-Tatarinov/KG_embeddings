...
def test_hf_loading():
    ...
    print(
        f"  Entities: {len(ent2id) if ent2id else train[:, [0, 2]].max().item() + 1}")
    print(
        f"  Relations: {len(rel2id) if rel2id else train[:, 1].max().item() + 1}")
    print()
    ...
    print(
        f"  Train: {expected_train} vs {len(train)} {'✓' if abs(len(train) - expected_train) < 100 else '✗'}")
    print(
        f"  Valid: {expected_valid} vs {len(valid)} {'✓' if abs(len(valid) - expected_valid) < 100 else '✗'}")
    print(
        f"  Test: {expected_test} vs {len(test)} {'✓' if abs(len(test) - expected_test) < 100 else '✗'}")

    nentity = len(ent2id) if ent2id else max_ent + 1
    nrelation = len(rel2id) if rel2id else max_rel + 1
    print(
        f"  Entities: {expected_entities} vs {nentity} {'✓' if nentity == expected_entities else '✗'}")
    print(
        f"  Relations: {expected_relations} vs {nrelation} {'✓' if nrelation == expected_relations else '✗'}")
    print()
