...
from dataset.kg_dataset import KGIndex, build_test_loaders, build_train_loaders, BidirectionalOneShotIterator
...
def test_rscf_wrapper(device="cpu"):
    ...
    model = create_model("TransE", nentity=nentity,
                         nrelation=nrelation, base_dim=8, gamma=12.0)
    ...
    dl_head, dl_tail = build_train_loaders(
        train_ids, nentity, nrelation, negative_size=3, batch_size=2,
        num_workers=0, use_filtered=True, all_true=all_true
    )
    ...
    for step in range(2):
        ...
        pos_loss = - \
            torch.nn.functional.logsigmoid(positive_score).squeeze(1).mean()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ckpt_path = save_checkpoint(
            tmpdir, "rscf_test", {"step": 2}, model, optimizer)
        model_new = create_model(
            "TransE", nentity=nentity, nrelation=nrelation, base_dim=8, gamma=12.0)
...
def test_mi_wrapper(device="cpu"):
    ...
    for step in range(2):
        ...
        optimizer.step()
        print(
            f"  Step {step+1}: kge_loss={kge_loss.item():.4f}, mi_loss={mi_loss.item():.4f}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ckpt_path = save_checkpoint(
            tmpdir, "mi_test", {"step": 2}, model, optimizer)
        model_new = create_model(
            "TransE", nentity=nentity, nrelation=nrelation, base_dim=8, gamma=12.0)
...
def test_med_wrapper(device="cpu"):
    ...
    for step in range(2):
        ...
        optimizer.step()
        print(
            f"  Step {step+1}: L_total={stats['L_total']:.4f}, L_ml={stats['L_ml']:.4f}, L_ei={stats['L_ei']:.4f}")
    ...
    metrics = evaluate_model(model.model, test_dl_head,
                             test_dl_tail, device=device)
    print(f"✓ Evaluation: MRR={metrics['mrr']:.4f}")

    # Test checkpoint
    with tempfile.TemporaryDirectory() as tmpdir:
        ckpt_path = save_checkpoint(
            tmpdir, "med_test", {"step": 2}, model, optimizer)
        base_model_new = create_model(
            "TransE", nentity=nentity, nrelation=nrelation, base_dim=8, gamma=12.0)
        model_new = MEDTrainer(base_model_new, d_list=[
                               2, 4, 8], submodels_per_step=2)
        load_checkpoint(ckpt_path, model_new)
        print("✓ Checkpoint save/load works")
...
def test_combined_wrappers(device="cpu"):
    ...
    model = MEDTrainer(base_model, d_list=[
                       4, 8], submodels_per_step=2, mi_lambda=0.1, mi_neg_size=3)
    model = model.to(device)
