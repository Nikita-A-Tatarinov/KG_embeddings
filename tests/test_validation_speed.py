...
print(
    f"  Validation loaders: head={len(v_head_full.dataset)} triples, tail={len(v_tail_full.dataset)} triples")
print(
    f"  Test loaders: head={len(t_head_full.dataset)} triples, tail={len(t_tail_full.dataset)} triples")
...
print(
    f"  Validation loaders: head={len(v_head_sample.dataset)} triples, tail={len(v_tail_sample.dataset)} triples")
print(
    f"  Test loaders: head={len(t_head_sample.dataset)} triples, tail={len(t_tail_sample.dataset)} triples")
...
print(
    f"  Sampled: {valid_sample:>6} triples ({100*valid_sample/valid_full:.1f}% of full)")
print(f"  Speedup: {valid_full/valid_sample:.1f}× faster validation")

print(f"\nTest (per direction):")
print(f"  Full:    {test_full:>6} triples")
print(
    f"  Sampled: {test_sample:>6} triples ({100*test_sample/test_full:.1f}% of full)")
...
print("=" * 70)
print(
    f"During training, validation will be ~{valid_full/valid_sample:.1f}× FASTER with 10% sampling!")
print(
    f"This saves {(valid_full-valid_sample)*2} triple evaluations per validation epoch.")
print(f"(×2 because we evaluate both head and tail prediction)")
print("=" * 70)
