...
def prepare_fb15k237(
    ...
    if use_hf:
        ...
        train, valid, test, ent2id, rel2id = load_kg(root, "FB15k-237", prefer_ids=prefer_ids)

    # Apply sampling if requested
    if sample_ratio is not None or max_entities is not None or max_triples is not None:
        print("\n=== Sampling FB15k-237 Dataset ===")
        print(f"Original sizes: train={len(train)}, valid={len(valid)}, test={len(test)}")
        ...
        print(f"Sampled sizes: train={len(train)}, valid={len(valid)}, test={len(test)}")
    ...
    all_true = KGIndex(triples_list=train.tolist() + valid.tolist() + test.tolist(), nentity=nentity, nrelation=nrelation)
    ...
    if sample_valid_ratio is not None and sample_valid_ratio < 1.0:
        ...
        print(
            f"  Additional validation sampling: {len(valid)} → \
                {len(valid_sampled)} triples ({sample_valid_ratio * 100:.0f}%)"
        )
